...
function middleware(
  ...
  // Return for testing
  return validate(req[readFrom], schema, options).then((value) => {
    // Set the validated and optionally cast values
    res.locals[writeTo] = {...res.locals[writeTo], ...value};

    next();
  }).catch(({details, message}) => next(badRequest(message, details)));
}
